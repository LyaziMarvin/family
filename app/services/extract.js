 ...
 * Returns the inserted record ID (used by upload pipeline + Chroma indexing).
 *
...
module.exports = async function extractFamilyDataFromFile(filePath, mimetype, userId) {
  try {
    ...
    // IMPORTANT: return the inserted record id
    const id = await insertRecord({ text, summary: JSON.stringify(summary) }, userId);
    return id;
  } catch (error) {
