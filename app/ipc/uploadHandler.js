...
function splitDocumentIntoCharChunks(text, maxTokensPerChunk = MAX_TOKENS_PER_CHUNK, ratio = TOKEN_CHAR_RATIO) {
...
ipcMain.handle('upload:files', async (_event, { docPath, photoPaths, musicPaths, token }) => {
  ...
  try {
    ...
    // Attempt to compute an automatic short summary/topic for the newly uploaded record.
    let autoAnswer = null;
    ...
    return {
      success: true,
      data: { docSavedTo: savedDocPath, fileName: originalDocName, recordId, autoAnswer, topic: topicNormalized }
    };
  } catch (err) {
